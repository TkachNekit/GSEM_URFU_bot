...
async def login(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        if isinstance(e, TooManyArgumentsInLogin):
            logging.error(
                f"User '{username}' unsuccessfully tried to authorize. Too many arguments were given."
            )
            message = "[Ошибка авторизации]    Токен был передан неверно    Необходимо: /login <TOKEN>"

        elif isinstance(e, NoArgumentsInLogin):
            logging.error(
                f"User '{username}' unsuccessfully tried to authorize. No arguments were given."
            )
            message = (
                "[Ошибка авторизации]    Не был дан токен    Необходимо: /login <TOKEN>"
            )

        elif isinstance(e, TokenAlreadyInUseError):
            logging.error(
                f"User '{username}' unsuccessfully tried to authorize. Token is already in use."
            )
            message = (
                "[Ошибка авторизации]    Был дан токен, под которым уже авторизованы"
            )

        elif isinstance(e, InvalidSessionToken):
            logging.error(
                f"User '{username}' unsuccessfully tried to authorize. Invalid token were given."
            )
            message = "[Ошибка авторизации]    Был дан несуществующий токен"

        elif isinstance(e, AlreadyLoggedInAccount):
            logging.error(
                f"User '{username}' unsuccessfully tried to authorize. User already authorized."
            )
            message = (
                "[Ошибка авторизации]    Пользователь уже авторизован с этого аккаунта"
            )

        else:
            logging.error(
                f"User tried to login and unpredicted error happened. Error: {e}"
            )
            message = (
                "[Ошибка авторизации]    Произошла непредвиденная ошибка при авторизации. "
...
async def login_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        logging.error(
            f"User '{username}' unsuccessfully requested login status. User is not authorized."
        )
...
async def progress(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        logging.error(
            f"User '{username}' unsuccessfully requested his progress on tasks. User is not authorized."
        )
        return "[Ошибка]    Необходима авторизация."
...
async def logout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        logging.error(
            f"User '{username}' unsuccessfully tried to log out. User is not authorized."
        )
...
async def students_downloader(
    ...
    try:
        ...
        logging.error(
            f"User '{username}' unsuccessfully tried to uploaded users' list. Wrong arguments."
        )
        return "[Ошибка]    Дата окончания действия токенов была указана неверно    Необходимо: 01.01.2023"

    except WrongDateFormatError:
        logging.error(
            f"User '{username}' unsuccessfully tried to uploaded users' list. Wrong date format."
        )
        return "[Ошибка]    Дата была дана в неверном формате   Необходимо: 01.01.2023"

    except InvalidDateError:
        logging.error(
            f"User '{username}' unsuccessfully tried to uploaded users' list. Wrong date."
        )
        return (
...
async def py_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        logging.warning(
            f"Downloaded user's file {received_file_name} as {token}\\{new_file_name}"
        )

        result = await run_tests(filepath, received_file_name)
        logging.warning(
        ...
        if isinstance(e, WrongAnswerError):
            ...
            logging.error(
                f"User {username} tried to send file with wrong file_name - {received_file_name}"
            )
            ...
            logging.error(
                f"User {username} sent file {received_file_name}. Error : {e}"
            )
            msg = (
                "[Ошибка исполнения программы]    Файл с программой запускается с ошибкой:\n\n"
                f"{str(e).split('/')[-1]}"
            )
        else:
...
async def upload_student_progress(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> str:
